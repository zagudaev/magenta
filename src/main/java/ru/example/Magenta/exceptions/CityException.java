...
public class CityException extends ResponseStatusException {
    ...
    public static void coordinatesAreBusyException(CityDTO cityDTO, CityRepository cityRepository) {
    ...
    public static void cityIDNotFoundException(CityRepository cityRepository, Long id){
        if (cityRepository.findById(id).orElse(null) == null){
            throw  new CityException(HttpStatus.BAD_REQUEST, "Город с таким ID не найден : "+
                    id);
        }
    }
    public static void cityIDNotFoundException(City city , Long id){
        if (city.getId() == null)
